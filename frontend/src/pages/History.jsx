...
const History = () => {
  ...
  const loadSessions = async () => {
    try {
      ...
      // Получаем реальные сессии из базы данных
      // Для этого сначала получим историю текущей сессии
      const currentSessionResponse = await chatAPI.getChatHistory(
        currentSessionId
      );
      const currentSessionData = currentSessionResponse.data;

      // Создаем сессию на основе текущих данных
      ...
      // В реальном приложении здесь должен быть endpoint для получения всех сессий
      // Пока используем только текущую сессию
      const realSessions = [currentSession];

      setSessions(realSessions);
    } catch (error) {
      console.error("Error loading sessions:", error);
      // Если ошибка, создаем базовую сессию
      setSessions([
        {
          session_id: currentSessionId,
  ...
  const loadSessionMessages = async (sessionId) => {
    try {
      ...
      // Преобразуем данные из API в формат сообщений
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => {
            clearChat();
            loadSessions(); // Перезагружаем сессии после очистки
          }}
